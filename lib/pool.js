...
function checkLogin(login, sendReply, callback)
{
	...
	if(login.indexOf('@') === 0)
	{
		...
		log('debug', logSystem, "checking %s for alias", [login]);
...
function getBlockTemplate(callback)
{
    ...
    var obj_to_rpc = {reserve_size: 8, wallet_address: config.poolServer.poolAddress, alias_details: alias_info, dev_bounties_vote: true};
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
   ...
   switch(method){
        case 'login':
            ...
            checkLogin(params.login, sendReply, function(error, result)
			{
				if (error !== false)
				{
					log('warn', logSystem, "login error: %j", [error]);
					sendReply('invalid address used for login');
					return;
				}
				params.login = result;
